...
- 仓库 README 负责说明“这是什么、如何安装、如何选择入口”
- 真正的默认运行契约应当以 `skills/` 下的 skill 本体为准
- 对支持 skill 安装的智能体来说，安装后应以 skill 作为主要行为来源，而不是只盯着 README
...
其中更严格、更可执行的版本已经写进主线 skill；这里保留的是仓库级摘要，主要用于那些不依赖 skill 自动加载、而是直接阅读仓库的智能体。
